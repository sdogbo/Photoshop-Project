...
int write_ppm(FILE *fp, const Image *im) {
  ...
  // confirm that we received good file and image handles
  if (fp == NULL || im == NULL) {
    free(fp);
    return -1;
  }
  ...
  // write in pixel data
  int num_written = fwrite(im->data, sizeof(Pixel), (im->rows) * (im->cols), fp);

  // (check this)
  if (num_written != (im->rows * im->cols)) {
    return -1;
  }
...
void free_image(Image **im) {
  ...
  // frees image data, frees image pointer, and sets image double pointer to NULL
  free((*im)->data);
  free(*im);
  im = NULL;
}
...
int resize_image(Image **im, int rows, int cols) {
  ...
  (*im)->data = realloc((*im)->data, ((*im)->rows * (*im)->cols) * sizeof(Pixel));
